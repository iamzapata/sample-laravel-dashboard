<div class="row well pest-categories">
    <div class="panel panel-primary">
        <div class="panel-heading">
            <h3 class="panel-title">Pest Categories</h3>
        </div>
        <div class="panel-body">
            <table class="table">
                <colgroup>
                    <col class="col-xs-3">
                    <col class="col-xs-3">
                    <col class="col-xs-6">
                </colgroup>
                <tr>
                    <th><u>Category Name</u></th>
                    <th><u>Type</u></th>
                </tr>
                <tr>
                    <td></td>
                    <td></td>
                    <td><a href="/admin/dashboard/#categories/create" class="btn btn-sm btn-success create-category">Create New</a>
                <!--
                <a href="/admin/dashboard/#categories/edit" class="btn btn-sm btn-primary edit-category">Edit</a>
                <a href="/admin/dashboard/#categories/delete" class="btn btn-sm btn-danger delete-category">Delete</a>
                -->
                    </td>
                </tr>
            </table>
        </div>
    </div>
</div>
